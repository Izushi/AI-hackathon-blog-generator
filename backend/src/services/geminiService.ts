...
4. 記事内容に関連する画像を1つ以上含める（placeholder画像URLを使用）
5. 関連するタグを3-5個提案
6. 読了時間を推定（分単位）
7. 文字数をカウント
8. ですます調を使わず、自分ごととして記述
...
注意：
- HTMLは適切にエスケープしてください
- 画像は<img>タグを使用し、以下の形式で含める：<img src="https://via.placeholder.com/600x400?text=[記事内容に関連するテキスト]" alt="[適切なalt属性]" class="w-full rounded-lg my-4" />
- 画像は記事の内容に応じて適切な位置に配置する
